...
var projectName = scope.projectName, formattedResource = "Project '" + scope.displayName + "'", context = {};
DataService["delete"]({
...
var cb = $scope.redirectAction();
cb && cb(encodeURIComponent($scope.project.metadata.name)), showAlert({
type:"success",
message:"Project '" + displayNameFilter(project) + "' was successfully updated."
});
}, function(result) {
$scope.disableInputs = !1, $scope.editableFields = editableFields($scope.project), showAlert({
type:"error",
...
}, angular.module("openshiftCommonServices").factory("APIService", [ "API_CFG", "APIS_CFG", "AuthService", "Constants", "Logger", "$q", "$http", "$filter", "$window", "gettext", "gettextCatalog", function(API_CFG, APIS_CFG, AuthService, Constants, Logger, $q, $http, $filter, $window, gettext, gettextCatalog) {
...
return "projectrequests" === resource || _.contains(resource, "/") || _.contains(REVIEW_RESOURCES, resource) ? !1 :!0;
...
return verbs ? _.contains(verbs, verb) || _.contains(verbs, "*") :!1;
...
}), angular.module("openshiftCommonServices").factory("ProjectsService", [ "$location", "$q", "AuthService", "DataService", "annotationNameFilter", "AuthorizationService", "gettext", "gettextCatalog", function($location, $q, AuthService, DataService, annotationNameFilter, AuthorizationService, gettext, gettextCatalog) {
...
return context.project = project, context.projectPromise.resolve(project), [ project, context ];
});
}, function(e) {
context.projectPromise.reject(e);
var description = gettextCatalog.getString(gettext("The project could not be loaded.")), type = "error";
return 403 === e.status ? (description = gettextCatalog.getString(gettext("The project")) + " " + context.projectName + " " + gettextCatalog.getString(gettext("does not exist or you are not authorized to view it.")), type = "access_denied") :404 === e.status && (description = gettextCatalog.getString(gettext("The project")) + " " + context.projectName + " " + gettextCatalog.getString(gettext("does not exist.")), type = "not_found"), $location.url(URI("error").query({
error:type,
